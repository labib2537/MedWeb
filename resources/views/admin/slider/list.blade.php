    ...
    <!-- Basic datatable -->
			...
			<div class="card">
					...
					<div class="text-left ml-2">
						<form action="{{route('slider_trash')}}" method="POST">
							@csrf
							<button type="submit" class="btn btn-danger btn-labeled btn-labeled-left btn-sm legitRipple"><b><i class="icon-trash"></i></b> Trash</button>
						</form>
					</div>
								...
								<td class="text-center">
									<div class="list-icons">
										<div class="dropdown">
											...
											<form action="{{ route('slider_show') }}" method="POST">
													@csrf
													<input type="hidden" name="id" value="{{$slider->id}}">
													<button class="dropdown-item" type="submit"><i class="icon-eye"></i> View</button>
						                        </form>

												<form action="{{ route('slider_updateStatus') }}" method="post">
													@csrf
												<input type="hidden" name="id" value="{{$slider->id}}">
												<input type="hidden" name="status" value="{{$slider->is_active}}">
												@if($slider->is_active==1)
